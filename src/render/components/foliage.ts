 ...
 * - **比例**「樹冠誇張的圓，樹幹短粗」→ 樹冠直徑 2.56 是樹高的 **0.715**；
 *   樹幹高 1.05（樹高的 0.293）而底徑 0.72，**高徑比只有 1.46**——短粗
 *   不是形容詞，是這個比值。以上全部由 `FOLIAGE_PROPORTIONS` 算出來，
 *   改常數就會跟著動，不會像這段註解一樣需要人手同步
 *
 * ## draw call 預算
 *
 ...
 * 每棵樹的葉瓣數。
 *
 * 這個數字直接乘上樹的數量進三角形預算，所以不能隨手加大。30 配
 * `LEAF_PAD_RADIUS` 0.46 的覆蓋率是 1.2；低於 1 圓穹底色會透出來，整顆樹
 * 從「交疊的葉瓣」退化成「圓球上的斑點」（第一版就是這樣，見下面那條）。
 */
