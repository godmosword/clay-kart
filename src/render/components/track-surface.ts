 ...
 * 路面（含疊層）的頂面高度，世界單位。
 *
 * **這個值必須被呼叫端用來墊高站在路上的東西。** R25 我把路面做成
 * 「厚度板 + 疊層」之後，路面頂就不在 y=0 了；但模擬給的車體 y 一直是 0，
 * 所以**車有 0.285 埋在路裡**（約四分之一個輪徑），而且沒有人發現——
 * 截圖裡輪子看起來偏矮就是這個，不是輪子做小了。
 *
 * 接地色塊也一樣：R32 加的那塊擺在 y=0.012，整個在路面底下，一次都沒顯示過。
 * 「車子沒有影子」的原因是它被路面蓋住，不是陰影參數錯。
 *
 * 匯出成常數而不是讓呼叫端自己抄數字——抄的那一份遲早會跟這裡分岔。
 */
export const ROAD_SURFACE_Y = SLAB_HEIGHT + OVERLAY_LIFT;
