...
export const TERRAIN = {
   ...
   * 樹幹。**取自黃金樣本，不是編的。**
   *
   * `CHARACTERS.md §6` 的地形表沒有樹幹色，而 `§5.6` 明文要求樹有「短粗」的
   * 樹幹——調色盤對 `§5.6` 是不完整的。最接近的既有 token 是
   * `contactShadow`，但那是陰影色，拿來當樹幹是語意錯用。
   *
   * 來源：`refs/clay/car-park.png` 兩棵樹的樹幹區域，242 個像素的明度中位數
   * 是 `#a2672d`（四分位 `#8d5923` / `#ba7b39`）。中位數是**算繪後**的顏色，
   * 已經含了光照；依 `§5.0` 的「略帶粉彩、比純色去飽和約 10–20%」退一階取
   * `#ab7a45`，線性空間飽和度 0.745，落在其餘 token 的帶內（最高 0.816）。
   */
  bark: 0xab7a45,
