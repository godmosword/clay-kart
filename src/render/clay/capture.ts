 ...
 * - 一張 512×512 的 2×2 合成（每格 256×256）：contact sheet 那一格用
 *
 * 這是 R18 的裁決，理由記在 `loop/round-18/TASK.md`。
 *
 * 讀畫布用 `toDataURL` 而不是 CDP 截圖：截圖會受視窗尺寸、DPR、捲動位置
 * 影響，畫布直讀拿到的就是渲染器實際輸出的那 512×512。
 */
import type { Group } from 'three';
import { VIEW_ORDER, VIEW_SIZE, createClayStage, mountSubject, type ViewName } from './stage.js';
...
export async function captureComponent(id: string, subject: Group): Promise<ComponentCapture> {
  ...
  try {
    ...
    return { id, views, sheetCell: sheet.toDataURL('image/png') };
  } finally {
    stage.dispose();
  }
}
