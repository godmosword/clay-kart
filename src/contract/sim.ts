...
export interface WorldInput {
   ...
   * 轉向輸入：-1 到 1。
   *
   * **`steer > 0` = 玩家看到的畫面往右轉，`steer < 0` 往左轉。**
   *
   * 這句話原本不在這裡，後果是 R20 才發現的：輸入端把 `ArrowRight` 對到
   * `+1`，物理端讓 `+1` 增加 yaw，而專案的 `forward = (sin yaw, cos yaw)`
   * 加上追尾相機沿 `+forward` 看出去時，yaw 增加是**畫面往左**。兩邊各自
   * 實作各自合理，合起來就是按右轉左。方向這種事只要契約沒寫，兩個獨立
   * 實作的工具就有一半機率對不起來——而且 typecheck 永遠是綠的。
   *
   * 這條定義以**玩家看到的畫面**為準，不以世界座標為準：世界座標的左右
   * 取決於相機在哪一邊，只有畫面是玩家真正感受到的東西。
   *
   * 目前的實作**不符合這條定義**（見 `loop/BACKLOG.md`「轉向在畫面上是反的」）。
   * 這裡先把定義釘住，修正歸屬依定義判斷，不是先改實作再回頭補文件。
   */
  steer?: number;
  /** 按住時強力減速。 */
  brake?: boolean;
