...
DROP_SECTIONS = (
    ...
    # §3 是**給 builder 的拍攝規範**，critic 一條都用不到，而它直接寫著
    # 「背景 #8a8a8a 中性灰」——等於告訴 critic「灰底那半邊是我們的」。
    #
    # 這條洩漏躲過了 R31 所有的修正（標籤、量化、構圖），因為它不在圖裡，
    # 在標準裡；而且從第一輪就在送。R31 的三個 critic 都引用了 §3 來說明
    # 兩個半邊的差異，其中一個直接寫「符合 §3 的拍攝規範」——它們讀得懂。
    re.compile(r'^## §3\b'),
    re.compile(r'^### §3\.'),
    # §0.1 說明「條款來源」與「比較對象」由不同素材擔任，並逐項列出
    # 參考半邊取自哪一批 —— 那是配對資訊。條文本體（§0）仍然送出，
    # 只有這一節不送。
    re.compile(r'^### §0\.\d'),
)

# 送出的副本必須含有這些章節，缺一即為切錯
REQUIRED = ('## §0 ', '## §1 ', '## §2 ', '## §4 ', '## §5 ', '## §6 ')

# 這些字樣若出現在副本裡，代表配對答案或評分機制外洩
FORBIDDEN = ('裁切', '暫緩', '校驗', '中位數', 'run1', '§7.1', '§7.2',
             '沒有參考半邊', '機械驗收', 'mechanical_checks_passed',
             # R31：§3 洩漏我們那半邊的背景色，整節已移除；這兩個字樣是它的指紋
             '8a8a8a', '中性灰',
             # R32：任何指出「哪一半邊是參考」的措辭都是配對資訊。
             # 我自己在 §0.1 與 §6 的註記裡各寫了一次，兩次都是這條擋下來的。
             '參考半邊', '角色 jpg', '角色照片')
...
§1.x 為評分流程說明、§3 為產圖端的拍攝規範，同樣與判斷無關，一併移除。
"""
