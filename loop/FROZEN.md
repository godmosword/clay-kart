...
> **🔓 開閘中（R35，Lead 裁決）—— `feel.py` 自 R22 關閘後第一次重開。**
>
> **原因：`§6.5` 在構造上不可能偵測到它要測的東西，真人試玩五分鐘就撞到。**
> `feel.py:585` 計卡牆幀要求 `collision_impulse > 0`，而 `world.ts:657` 在
> 「已穿透但無向外速度」（正是頂著牆不動的狀態）明文把它設為 0。
> 兩個各自正確的改動互相抵消，`wall_stick_frames` 永遠計到 0——
> 而 R34 收尾時它顯示 `= 2`、PASS，46/46 全綠。
>
> **開閘範圍見 `loop/round-35/TASK-codex.md`。准許三件：`§6.5` 改成
> 「持續貼邊界且速度近 0 的連續幀數」（與新撞擊無關）、倒車不再需要同時按油門、
> 貼牆靜止時可轉向。**
>
> **明文禁止：不得放寬 `§6.5` 的窗口 `[0, 3]`。** 這一輪的目的是讓它量到
> 真實情況，很可能因此 FAIL——那就是正確結果。不得動其餘 45 項、
> 不得新增 fallback、不得為此重新平衡漂移或加速。
>
> **收尾條件**：46 項逐項對照 R22／R34 貼出（`§4.10` 餘裕僅 0.00028），
> 附可重現卡死的 probe，artifact commit 進 `loop/round-35/artifacts/`。
> 收尾後立即關閘，不需再次裁決。
