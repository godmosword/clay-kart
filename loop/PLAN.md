...
**「12 元件全部 ≥4 分才進 W4」這道閘從現在起不生效。**

36 輪、11 天之後的實際狀態：賽道 1 條、道具 0、音效 0 個檔、
角色實作 1/6、視覺元件 6/12 全部 3 分、無結算畫面、無選單。
**能做的事就是在一個圈上開三圈，然後什麼都不會發生。**

#### 為什麼會這樣——四份 bar 沒有一項會因為「東西不存在」而失敗

`BAR-FEEL` 46 項、`BAR-PERF` 17 項、`BAR-VISUAL` 12 元件，
**全部都在問「已經存在的東西夠不夠好」**。沒有任何一條檢查會因為
沒有音效、沒有道具、沒有第二條賽道、沒有結算畫面而變紅。

所以這個 loop 可以在 `46/46 + 15/17` 的狀態下永遠跑下去，
而完全不會注意到這不是一個遊戲。

**這跟本專案抓了十一次的缺陷是同一個形狀——檢查不可能失敗。**
只是這一次它在流程層，而且是最貴的一個：它決定了 36 輪的走向。

第二層原因是這道閘本身的序列化：W3 要 12 元件 ≥4 才進 W4，
現在 6 個元件、都是 3 分，**這個順序保證了長時間沒有可玩內容**。

#### 處置

- W3 的視覺 loop **暫停**，6 個元件維持現狀（視覺上不好看但連貫）
- `BAR-VISUAL` 不廢除也不降低門檻——**它是暫停，不是失效**，
  W4 的最小可玩集完成後恢復
- 進行中的量測修正（`§4` 的 tier probe 污染）由 Codex 收完即止，
  **不要再開新的量測輪**
- 新增 `BAR-CONTENT.md`：一份**會失敗**的內容完整度檢查（見下）

---

## W4 — 最小可玩集（R36 起，優先於 W3 剩餘工作）

**目標不是「做完」，是「像個遊戲」。** 判準：一個沒看過這個專案的人
打開來玩，五分鐘內會知道自己在玩什麼、有沒有贏。

| # | 項目 | 工具 | 為什麼是它 |
|---|---|---|---|
| 1 | **結算畫面** | Cursor | 現在跑完三圈什麼都不會發生。這是「不是遊戲」最直接的證據 |
| 2 | **音效** | Claude Code | `CHARACTERS.md §7` 說是「最省事的一塊」，上游 `podcast-website/public/stories/*/audio.mp3` 可 100% 複用，**而它從 W3 開始就該做，一直沒做** |
| 3 | **道具箱 + 2–3 種道具** | Codex（物理）+ Claude Code（視覺） | 卡丁車遊戲沒有道具就只是繞圈 |
| 4 | **第二條賽道** | Codex | 一條環形跑 36 輪，任何人都會膩 |
| 5 | **其餘車手** | Claude Code | `CHARACTERS.md` 寫了 6 位，實作 1 位 |

**順序即優先序。** 1 和 2 是一兩輪的事，先做完那兩件再談 3–5。

### W4 的完成判準

不是分數，是 `BAR-CONTENT.md` 全綠 —— 那份檢查的每一條都是
「這個東西存在嗎、會不會動」，**答案是 false 就 FAIL**，沒有窗口可以調。
