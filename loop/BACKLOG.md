...
### 輸入來源沒有接線，車目前不可操控
- **輪次**：R1
- **現況**：`src/physics/world.ts` 提供 `setInput()`，但全 repo 沒有任何呼叫端。
  `src/loader/bootstrap.ts` 的主迴圈只呼叫 `step()` 與 `snapshot()`
- **目標**：W1 的定位是「可玩骨架」，玩家要能用鍵盤／觸控操控
- **落差**：目前車自己沿賽道跑，不接受輸入
- **歸屬問題**：輸入處理屬 `src/ui/` 或 `src/loader/`（Cursor 範圍），
  但介面契約在 `bootstrap.ts`。**Lead 拆 W1 時沒有指派這件事**，
  三個工具的任務書都沒寫到，所以沒有人做
- **來源**：Lead R1 審查
- **狀態**：待裁決——需決定歸 Cursor 或併入 loader 契約一起處理
