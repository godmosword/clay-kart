...
- **相對穩定的是偏好方向**：`kart-body` 與 `driver-face` 兩輪都判參考較優。
  那個訊號一致，可以拿去改；分數本身不行
- **可能的處置（都是裁決，不自決）**：同一張 sheet 跑 N 次取中位數／改用
  成對偏好（哪一邊較好）而非絕對分數當門檻／兩者併用（偏好定方向、
  多輪中位數定分數）。跑 N 次要花 N 倍 token，值不值得是預算判斷
- **狀態**：待裁決——在有結論之前，任何單輪分數都不該拿來宣告元件通過或退步
...
- **影響**：`track-surface` 這一組的分數在配對修正前沒有意義。**不能據此說
  這個元件沒做好**——它贏了自己的參考
- **建議**：`refs/clay` 裡若沒有奶油沙色的純路面特寫，就比照 `item-boxes`
  改列暫緩，不要拿違反條文的圖當標準。同時 R20 那 10 組配對值得整批用
  「參考半邊自己合不合 §5.x」重新過一遍，而不是等 critic 一組一組指出來
- **狀態**：待裁決
