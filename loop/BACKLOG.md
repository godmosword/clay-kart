...
- **可能的方向**（都需要裁決）：
  ...
  3. **改成單邊評分。** 不比較，直接照 `§5` 條款給我們的圖打分。`§1.3` 的
     機械驗收已經是這個形狀
- **狀態**：待裁決——這是 R31 唯一沒有修法的一項，也是最根本的一項
...
- **這是第三次**：R28 是 `§5.5` 指名一個 `§6` 禁止的顏色，R30 是 `§5.6` 說錯
  ...
  原始素材才發現
- **要裁決的是**：`§6` 的 0.92 是對算繪產出的約束，還是對「黏土世界」整體的
  約束？如果是前者，條文要寫明它不適用於參考素材，`§1.3` 的機械檢查也要跟著
  分流；如果是後者，那 `§7` 的素材選錯了
- **狀態**：待裁決
...
- **可能的解**：兩種都是這個 IP 裡合法的黏土樹，差在尺度——黃金樣本的樹是
  背景小道具（264px 的圖裡約 55px），`安安救護車.jpg` 的是前景主體微距。
  `§5.6` 或許該同時容納兩種並寫明何時用哪一種
- **狀態**：待裁決
