...
- **這是我自己造成的**：為了保護一個已知不成立的盲測，去限制 critic 使用標準
  裡最重要的一條
- **可能的解**：既然已經承認 R32 是「公平比較」不是盲測，那條指示應該拿掉，
  改成明說「其中一邊是即時算繪、一邊是實拍黏土，`§5.0` 問的正是前者離後者
  多遠」。這與「盲測不可能成立」那條裁決是同一個方向的延伸
- **狀態**：待裁決
...
- **所以**：這一條在這個工具上**依其構造永遠無法被判定通過**——不是沒做到，
  ...
  **有一條檢查不可能通過**
- **可能的解**：`§5.3` 的那半句需要一張**整車視角**的圖才判得了，
  而 `game-shot.mjs` 已經在產那種圖。要嘛把它納入 critic 的材料，
  要嘛把那半句移到別的驗收路徑
- **狀態**：待裁決
