...
### R35 撞牆卡死已修，但換來兩項新 FAIL——待裁決

- **輪次**：R35（`94394b6`，Lead 獨立驗證重現一致）
- **修好了**：`§6.5` 的定義改成直接讀 `wall_contact` telemetry，
  不再從 `collision_impulse` 推導（那正是被刻意歸零的訊號）。
  probe：撞擊 tick 255、靜止貼牆 tick 352，**只按倒車 1 幀後速度 0.03437、
  只按轉向 1 幀後 yaw delta 0.00522**。卡死解除
- **代價（Lead 獨立重跑，與 Codex 逐項一致）**：

  | | R34 | R35 | 窗口 | |
  |---|---|---|---|---|
  | `§6.3 collision_recovery_time_s` | 0.279 | **0.183** | `[0.20, 0.45]` | 低於下限 |
  | `§4.7 car_lengths_gained_tier3` | 3.595 | **5.873** | `[2.8, 4.0]` | 高於上限 47% |

  `§4.10` 十位數不變（`0.9697186325`）、`§4.5`／`§6.5` 不變
- **兩項的性質不同，不該一起處置**：
  - **`§6.3` 低於下限，可能是「修好了」而不是「壞了」**。窗口的下限存在
    是因為瞬間回復不真實；現在 0.183s 比設計意圖更快，但那正是
    「不再黏在牆上」的直接後果。**這條要判的是窗口下限本身還適不適用**
  - **`§4.7` 高於上限 47%，這是真的行為改變**。tier-3 漂移的獲益從 3.6 個車身
    變成 5.9 個，而 `§4.10`／`§4.5` 都沒動——為什麼只有 tier3 受影響，
    根因未查。**這條在查清楚之前不該接受**
- **這正是任務裡警告的事**：轉向與速度耦合，動碰撞會推掉別的。
  Codex 誠實回報沒有隱瞞，也沒有去調窗口讓它變綠——那是對的做法
- **狀態**：待裁決。`§4.7` 的根因要先查（建議：tier3 probe 的路徑是否
  原本會擦到牆而被減速）

### 裁決（R35）：`§4` 的 tier probe 一直是沿著牆量的——窗口不動，改 probe

- **輪次**：R35。Codex 的逐 tick 軌跡（`202a04e`）證偽了我的假設，
  **而證偽的方式比假設成立更有價值**
- **我的假設**：tier3 原本擦牆被減速、R35 後不擦所以跑更遠 → 環境變了
- **實測推翻**：**兩版都擦牆**。首次物理分歧在 tick 265 的 charging 期間，
  兩版當時都 `wall_contact = true`、距邊界 0、地面速度約 20.674。
  變的不是「黏不黏牆」，是邊界重解析改變了擦牆的軌跡：

  |  | release 時距邊界 | 首次接觸 | 窗內接觸幀 | 收益 |
  |---|---|---|---|---|
  | tier3 R35 前 | 1.6176 | tick 570 | 177 | 3.5947 |
  | tier3 R35 後 | 0.3809 | tick 557 | 190 | 5.8727 |

- **所以 `5.873` 與 `3.595` 都不是乾淨的漂移量測**（Codex 自己的結論，我同意）
- **而軌跡還顯示了更嚴重的一件事**：`tier2` 的量測窗內也有
  **204 個接觸幀**（tick 398–678）。它的數字前後完全不變
  （`1.5603197783466292` 兩版逐位元相同），**但不變不等於有效**——
  它只是剛好沒被這次的邊界重解析擾動到
- **`§4.5` 是 `BAR-FEEL §9` 指定的唯一硬門檻，而它一直是沿著牆量出來的。**
  從 R5 到 R34，`§4` 的 tier 收益從來沒有在無牆環境下被量過

#### 裁決

1. **窗口全部不動**（`§4.5`／`§4.7`／`§6.3`）。在拿到乾淨量測之前改窗口，
   等於把污染合理化
2. **不改漂移物理**。目前沒有證據說漂移本身有問題
3. **改 probe**：tier1／2/3 的量測窗內必須 `wall_contact` 全為 false。
   probe 若做不到零接觸，**要誠實 FAIL 而不是照樣輸出數字**
   （跟 `§5.2` 的 `incomplete_five_lap_run` 同一個形狀）
4. 拿到乾淨數值之後**才**重新檢視三個窗口是否還合理——很可能要重訂，
   因為現有窗口是照污染過的數字調出來的
- **`§6.3` 一併押後**：它也是從牆碰撞量的，同一個污染問題要先排除
- **這是同一家族的第十一例，但形狀是新的**：前十例是「檢查不可能失敗」或
  「檢查不可能通過」，這次是**檢查一直在量另一件事**——而且它綠了三十輪
- **狀態**：已裁決，待開任務給 Codex（改 probe）

### R36 裁決：W3 閘門暫停，改做最小可玩集，新增 `BAR-CONTENT.md`

- **輪次**：R36。**觸發者是使用者，不是任何一條檢查**——
  「開發進度完全不如預期，可玩性很低」
- **實際狀態**（36 輪、11 天）：賽道 1 條、道具 0、音效 0 個檔、
  角色實作 1/6、視覺元件 6/12 全 3 分、無結算畫面、無選單。
  而同時 `BAR-FEEL` 46/46、`BAR-PERF` 15/17
- **根因——四份 bar 沒有一項會因為「東西不存在」而失敗**：
  46 + 17 + 12 條檢查**全部在問「已經存在的東西夠不夠好」**。
  沒有任何一條會因為沒有音效、沒有道具、沒有第二條賽道、沒有結算畫面而變紅。
  **所以 loop 可以全綠地跑 36 輪，而完全不會注意到那不是一個遊戲**
- **這是第十一種「檢查不可能失敗」，也是最貴的一種**：
  前十種讓某個指標失去鑑別力，這一種讓**整個開發方向**失去回饋
- **第二層原因**：`PLAN.md` 要求 W3 的 12 元件全部 ≥4 才進 W4。
  現在 6 個元件、都是 3 分——**這個序列化本身保證了長時間沒有可玩內容**
- **我要認的部分**：那些輪次的優先序是我排的。每個決定單獨看都站得住腳
  （R17–R29 的視覺分數確實是廢的、`§4` 的量測確實從 R5 就髒了），
  **但加起來的比例是錯的**。我把「把量測做對」當成目標本身，而它只是手段
- **裁決**：
  1. W3 視覺 loop **暫停**（不是廢除，不是降門檻）。6 元件維持現狀
  2. 進行中的 `§4` tier probe 污染修正由 Codex 收完即止，
     **不要再開新的量測輪**
  3. 改做 W4 最小可玩集，順序即優先序：
     **結算畫面 → 音效 → 道具箱 → 第二條賽道 → 其餘車手**
  4. 新增 `BAR-CONTENT.md`：布林值、**沒有窗口可調**、
     未實作就是 FAIL。這是唯一能防止同樣漂移再發生的機制
- **`BAR-CONTENT` 刻意不含品質判斷**（好不好看／手感／效能／平衡）——
  混進去會讓它變得可以爭辯，而它的價值就在於不可爭辯
- **音效這條特別要記**：`CHARACTERS.md §7` 寫著「上游可 100% 複用，
  是最省事的一塊」，`PLAN.md` 寫著「音效在 W3，不是 W4」。
  **從 W3 開始就該做，36 輪一個檔都沒有。** 上游資產確認在
  `podcast-website/public/stories/*/audio.mp3`
- **狀態**：已裁決並落地（`PLAN.md` 改寫、`BAR-CONTENT.md` 新增、
  `loop/round-36/TASK-cursor.md` 已開）

### 裁決（R36）：`§4` 三個 tier 的乾淨值全部落在窗口內；`§6.3` 的下限降到 0.15

- **`§4` 乾淨量測（Codex `9e418ac`，Lead 獨立重跑一致）**：

  | | 污染值 | 乾淨值 | 窗口 | |
  |---|---|---|---|---|
  | `§4.5` tier2 | 1.560 | **1.822** | `[1.5, 2.5]` | PASS |
  | `§4.6` tier1 | 0.609 | **1.098** | `[0.6, 1.1]` | PASS |
  | `§4.7` tier3 | 5.873 | **3.116** | `[2.8, 4.0]` | PASS |

  **窗口本來就是對的，是量測環境把數字扭曲了。** `§4.6` 原本餘裕只有
  1.88%，乾淨值一量出來就落在中段——那 1.88% 是污染造成的假警訊。
  `contact_frames_in_window` 已成常設欄位，下次有人動碰撞會立刻看得出來
- **`§6.3` 裁決：下限 `0.20 → 0.15`**。實測 `0.183`，原本 FAIL
  - **這是我在 `TASK-codex-3` 裡明文禁止 Codex 做的事**（不得調窗口讓它變綠）。
    我現在做它，差別在於**根因已經查清楚**：0.279 → 0.183 是 R35 撞牆逃脫
    修正的直接後果，不是漂移或碰撞參數被動過
  - **那個 `0.20` 下限編碼的是症狀不是意圖**。它是在「車會黏在牆上」的年代
    調出來的，量到的是黏著本身。現在車能立刻脫困，0.183s 仍然不是瞬間，
    而「撞牆後 0.18 秒恢復控制」正是使用者要的
  - 上限 `0.45` 不動——那一端擋的是「撞一下要等半秒」，意圖仍然有效
- **`BAR-FEEL` 回到 46/46**，而且這次的 46 比 R34 的 46 誠實：
  其中三項不再是沿著牆量出來的
- **狀態**：已裁決並落地

### R36 交件驗證：道具系統與結算畫面都通過，但過程抓到三件

- **輪次**：R36，Codex `841327c`／`fc7d11f`、Cursor `19d01a1`，Lead 獨立驗證
- **Codex 道具系統（`BAR-CONTENT §2.5`／`§2.6`）——乾淨**：
  - **三次 ghost-replay 位元相同**（md5 `9d88bd9b…` ×3），決定性沒破
  - **`feel.py` 46/46 PASS**，`§4.10 = 0.9697186325` 十位數不變
    ——那個 0.00028 的餘裕活下來了
  - 只有三項動了，**全在 `§12`（AI 行為），方向都合理**：
    `12.2 ai_overtake_time_s` 3.633 → 2.233（AI 有道具，超車變快）、
    `12.3` 3.017 → 3.15（**餘裕從 0.098% 變成 5%**）、`12.4` 1.0032 → 1.0065
  - 四個道具箱、拾取半徑 2.2、重生 360 tick；加速 +8 持續 1.25s、
    震波 0.55× 持續 1.5s。**加速走獨立的 speedLimit 加成，沒有跟 mini-turbo
    共用 kick**——任務明文要求的那條有做到
- **Cursor 結算畫面（`§2.1`／`§2.2`）——通過**：完賽 22.02s、名次 `4/4`、
  四列排名（玩家那列標 `(YOU)`）、重新開始鈕存在且未 disabled

#### 一、兩邊都 commit 了但沒 push，而我的收件匣只看 `origin/`

上線當天就漏了一次。**檢查了最容易檢查的位置（遠端 ref），不是工作實際
所在的位置（builder 的工作目錄）**——第十二次同一個形狀，而且這次是我
一輪前才寫的工具。已改成一律讀本地 worktree 的 `HEAD`，另外標示推了沒有，
並且報未提交的改動。

#### 二、`check-race-result.mjs` 是 flaky 的

整合後第一次跑 FAIL（`race-result never appeared within 120000ms`），
之後連跑三次全 PASS。**在 Cursor 自己的分支上第一次就 PASS。**

我一度判斷成「整合造成的回歸」——那個判斷是錯的，先在對方分支上重跑才
分辨出來。但 flaky 本身是真的缺陷：**這個專案的歷史上，不穩定的量測
造成的傷害不亞於錯誤的量測**（R28 的 `0.80` 離群值壓了五輪、
R33 的 steer rate 有 ±25% 變異）。已列待處理。

#### 三、收件匣的「越界」警告有偽陽性

它用 `git diff --name-only main <head>` 判斷 builder 動了誰的檔，
而那個 diff **包含刪除**——builder 分支只是沒有 main 後來新增的檔案，
就會被報成「動到 `src/render/`」。訊號本身不假（那些檔確實有差異），
但歸因錯了。「會刪掉 N 個檔」那條是對的，實際擋下也是靠它。待修。
