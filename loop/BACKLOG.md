...
- **狀態**：待裁決——處置已完成（`43efe59`、`c155b8e` 兩次都由 Lead 補併），
  但連兩輪發生，制度化那道檢查的理由已經很充分：收尾時若 `loop/round-N/`
  出現新的 `VERDICT-*.json`，就用 main 自己的程式碼重跑一次逐項比對
...
- **狀態**：待裁決——不阻擋收尾，但 `4.10` 的 0.03% 餘裕值得決定是要接受、
  調窗口、還是換一個不那麼靠邊的阻力值
